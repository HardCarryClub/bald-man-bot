...
export const pattern = `${PATTERN_BASE}-cannot-host`;
...
export default async function (
  ...
  if (block.responses.cannotHost.find((entry) => entry.userId === interaction.user.id)) {
    return interaction.editReply({ content: "You have already marked yourself as can't host for this time block." });
  }

  block.responses.cannotHost.push({ userId: interaction.user.id, note: null });
